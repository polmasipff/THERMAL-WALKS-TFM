...
3) Kruskal-Wallis H is computed with scipy.stats.kruskal, i.e. with tie correction; windows where all coordinate values are identical are assigned eps^2=0.
...
def kruskal_eps2(x: np.ndarray, lab: np.ndarray, min_win: int, min_group: int, clip_zero: bool = True) -> float:
    ...
    if any(len(g) < min_group for g in groups):
        return np.nan
    # If all values are identical, there is no rank separation.
    # scipy.stats.kruskal raises ValueError in this exact case, but the
    # scientifically correct contribution to separability is zero, not a crash.
...
def prepare_data(
    ...
    # Reset index so numpy positional arrays and walk bootstrap indices agree.
