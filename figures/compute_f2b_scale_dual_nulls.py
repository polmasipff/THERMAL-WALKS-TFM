...
5) a coordinate-specific discrimination floor table for each null.
...
def compute_floor_table(results: pd.DataFrame, require_ci: bool, pooled_null: bool) -> pd.DataFrame:
    ...
    Two floors are useful:
    - obs_floor: first delta where obs > null95.
    - robust_floor: first delta where plotted lower bootstrap band > null95.
    """
    rows = []
    for null_name in ["global", "within"]:
        ...
        for coord in results["coord"].drop_duplicates():
            ...
            for _, r in s.iterrows():
                ...
                if not np.isfinite(obs_floor) and np.isfinite(r["obs_plot"]) and r["obs_plot"] > threshold:
                    obs_floor = float(r["delta"])
                if not np.isfinite(robust_floor) and np.isfinite(r["lo_plot"]) and r["lo_plot"] > threshold:
                    robust_floor = float(r["delta"])
            rows.append({
                "coord": coord,
                "null": null_name,
                "threshold_type": "pooled_max_across_coords" if pooled_null else "coordinate_specific",
                "obs_floor_delta": obs_floor,
                "robust_floor_delta": robust_floor,
                "obs_floor_text": f"> {s.delta.max():.2f} SD" if not np.isfinite(obs_floor) else f"{obs_floor:.2f} SD",
                "robust_floor_text": f"> {s.delta.max():.2f} SD" if not np.isfinite(robust_floor) else f"{robust_floor:.2f} SD",
            })
    return pd.DataFrame(rows)
...
def plot_f2b(
    ...
    ax.set_title(f"separability scale ({title_null} null, {mode} band; {pooled_txt})")
...
def plot_f3_contrast(prep: PreparedData, floors: pd.DataFrame, outdir: Path, null_name: str, floor_kind: str = "robust") -> pd.DataFrame:
    ...
    ax.set_xticklabels([disp_for(c) for c in prep.coords], fontsize=12)
    ax.set_ylabel("within-walk range (SD units of each coordinate)")
    ax.set_title(f"within-walk contrast vs coordinate-specific {null_name} floors")
